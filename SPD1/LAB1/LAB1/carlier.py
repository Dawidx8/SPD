...
def Carlier(tasks,NUMBER):
    ...
    if U < ub:
        ...
        #print(ub)
        #print([i.id for i in pi])

    b=max(pi,key=operator.attrgetter('cq'))
    b_index=pi_optymalne.index(b)
    ...
    for i in range(c_index+1,b_index+1):
        K.append(pi_optymalne[i])
    
    r=min([j.r for j in K])
    q=min([j.q for j in K])
    p=sum([j.p for j in K])

    r_pi=pi_optymalne[c_index].r
    r_pi_old=r_pi

    pi_optymalne[c_index].r=max(r_pi,r+p)
    ...
    pi_optymalne[c_index].r=r_pi_old

    q_pi=pi_optymalne[c_index].q
    q_pi_old=q_pi
    pi_optymalne[c_index].q=max(q_pi,q+p)
    ...
    pi_optymalne[c_index].q=q_pi_old
